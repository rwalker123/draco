...
namespace SportsManager
{
    public class CustomMobileViewEngine : IViewEngine
    {
        ...
        public ViewEngineResult FindPartialView(ControllerContext context, string viewName, bool useCache)
        {
            ...
            {
                return BaseViewEngine.FindPartialView(context, PathToSearch + "/" + viewName, false /*useCache*/);
            }
        ...
        public ViewEngineResult FindView(ControllerContext context, string viewName, string masterName, bool useCache)
        {
            if (IsTheRightDevice(context))
            {
                return BaseViewEngine.FindView(context, PathToSearch + "/" + viewName, masterName, false /*useCache*/);
            }
            return new ViewEngineResult(new string[] { }); //we found nothing and we pretend we looked nowhere
        }
    ...
    public static class MobileHelpers
    {
        ...
        {
            return IsMobileDevice(c.HttpContext.Request);
        }
        ...
        public static bool IsMobileDevice(HttpRequestBase request)
        {
            return (request.Browser.IsMobileDevice ||
                ...
                request.UserAgent.IndexOf("BlackBerry", StringComparison.OrdinalIgnoreCase) > 0 ||
                request.UserAgent.IndexOf("Fennec", StringComparison.OrdinalIgnoreCase) > 0 ||
                request.UserAgent.IndexOf("Mobile", StringComparison.OrdinalIgnoreCase) > 0);
        }

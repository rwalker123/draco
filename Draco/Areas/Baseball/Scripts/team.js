function initTeamViewModel(accountId, teamId, teamName, isAdmin) {

    var teamElem = document.getElementById("teamInfo");
    if (teamElem) {
        var teamVM = new TeamViewModel(accountId, teamId, teamName, isAdmin);
...
self.TeamViewModel = function (accountId, teamId, teamName, isAdmin) {
    ...
    self.saveTeamName = function () {
        ...
        var url = window.config.rootUri + '/api/TeamAPI/' + self.accountId + '/Team/' + self.teamId + '/teamname';
        $.ajax({
            type: "PUT",
            data: {
                Id: self.teamId,

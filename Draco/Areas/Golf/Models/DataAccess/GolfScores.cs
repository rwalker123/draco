using System;
using System.Collections.Generic;
using System.Linq;
using SportsManager;
using SportsManager.Model;

namespace DataAccess.Golf
{
	/// <summary>
	/// Summary description for GolfScores
	/// </summary>
	public static class GolfScores
	{
        public static void GetWeeklyActualLowScoresLeaders(long flightSeasonId)
        {

        }

		/// <summary>
		/// For the given account, update the start index for each golf score, for each contact.
		/// </summary>
		/// <param name="accountId"></param>
		public static void UpdateAllStartIndex(long accountId)
		{
            DB db = DBConnection.GetContext();

			var contactIds = (from c in db.Contacts
							  where c.CreatorAccountId == accountId
							  select c.Id);

			foreach (var contactId in contactIds)
			{
				var golfScore = (from gs in db.GolfScores
								 where gs.ContactId == contactId
								 orderby gs.DatePlayed ascending
								 select gs).FirstOrDefault();

				if (golfScore != null)
				{
					// first score for player, so the start index must be 0.
					golfScore.StartIndex = null;
					golfScore.StartIndex9 = null;
					UpdateIndexForSubsequentScores(golfScore);
					db.SubmitChanges();
				}
			}
		}

		/// <summary>
		/// Each golf score keeps a record of the players handicap index when the round started, this
		/// is to allow ESC calculations. This is a problem because the start index is based on the
		/// previous scores, so if a previous score changes, all subsequent start indexes are invalid.
		/// This method will recalculate the start index of scores entered after the given score.
		/// </summary>
		/// <param name="s"></param>
		public static void UpdateIndexForSubsequentScores(GolfScore s)
		{
            DB db = DBConnection.GetContext();

			double? startIndex = CalculateHandicapIndexOnDate(s.ContactId, s.DatePlayed);
			double? startIndex9 = CalculateHandicapIndexOnDate(s.ContactId, s.DatePlayed, for9Holes: true);

			// get list of next matches ordered by date.
			var futureMatches = (from gs in db.GolfScores
								 where gs.ContactId == s.ContactId && gs.DatePlayed > s.DatePlayed
								 orderby gs.DatePlayed ascending
								 select gs);

			foreach (GolfScore golfScore in futureMatches)
			{
				golfScore.StartIndex = startIndex;
				golfScore.StartIndex9 = startIndex9;
				startIndex = CalculateHandicapIndexOnDate(s.ContactId, golfScore.DatePlayed);
				startIndex9 = CalculateHandicapIndexOnDate(s.ContactId, golfScore.DatePlayed, for9Holes: true);
			}

			db.SubmitChanges();
		}

		public static IEnumerable<GolfScore> GetRecentGolfScores(long contactId, int numScores)
		{
            DB db = DBConnection.GetContext();

			return (from gs in db.GolfScores
					where gs.ContactId == contactId
					orderby gs.DatePlayed descending
					select gs).Take(numScores);
		}

		public static IEnumerable<GolfScore> GetGolfScores(long contactId)
		{
            DB db = DBConnection.GetContext();

			return (from gs in db.GolfScores
					where gs.ContactId == contactId
					orderby gs.DatePlayed descending
					select gs);
		}

		public static GolfScore GetGolfScore(long id)
		{
			GolfScore golfScore = null;

            DB db = DBConnection.GetContext();

			golfScore = (from gs in db.GolfScores
						 where gs.Id == id
						 select gs).SingleOrDefault();

			return golfScore;
		}

		/// <summary>
		/// Utililty to hold 2 9 hole score values for calculating a handicap.
		/// </summary>
		private class GolfScoreDouble
		{
			public GolfScore Score1 { get; set; }
			public GolfScore Score2 { get; set; }

			public GolfScoreDouble(GolfScore score1)
			{
				Score1 = score1;
				Score2 = null;
			}

			public GolfScoreDouble(GolfScore score1, GolfScore score2)
			{
				Score1 = score1;
				Score2 = score2;
			}

			public double? m_differential = null;
			public double Differential
			{
				get
				{
					if (m_differential == null)
					{
						// ratings should return 9 hole rating since that is what was played,
						// take the average of the slopes.
						m_differential = GolfScore.CalculateDifferential(Score1.TotalESCScore(true) + Score2.TotalESCScore(true), Score1.GetRating() / 2.0 + Score2.GetRating() / 2.0, (Score1.GetSlope() + Score2.GetSlope()) / 2);
					}

					return m_differential.Value;
				}
			}
		}

		public static double CalculateScoringAvg(long contactId)
		{
			double avg = 0.0;

            DB db = DBConnection.GetContext();

			var last40Rounds = (from gs in db.GolfScores
								where gs.ContactId == contactId
								orderby gs.DatePlayed descending
								select gs).Take(40);

			GolfScore pending9HoleScore = null;
			int numRounds = 0;
			long totalScore = 0;

			foreach (GolfScore gs in last40Rounds)
			{
				if (numRounds == 20)
					break;

				bool is9Holes = gs.HolesPlayed != (int)GolfScore.eHolesPlayed.Eighteen;
				if (is9Holes)
				{
					if (pending9HoleScore != null)
					{
						totalScore += gs.TotalScore + pending9HoleScore.TotalScore;
						numRounds++;
						pending9HoleScore = null;
					}
					else
					{
						pending9HoleScore = gs;
					}
				}
				else
				{
					totalScore += gs.TotalScore;
					numRounds++;
				}
			}

			if (numRounds > 0)
			{
				avg = totalScore / numRounds;
			}


			return avg;
		}


		public static double? CalculateHandicapIndexOnDate(long contactId, DateTime beforeDate, bool for9Holes = false)
		{
			double? handIndex = null;

            DB db = DBConnection.GetContext();

			if (for9Holes)
			{
				var last20Rounds = (from gs in db.GolfScores
									where gs.ContactId == contactId && gs.DatePlayed <= beforeDate && gs.HolesPlayed == 9
									orderby gs.DatePlayed descending
									select gs).Take(20);

				handIndex = CalculateHandicapIndex9(last20Rounds);
			}
			else
			{
				var last40Rounds = (from gs in db.GolfScores
									where gs.ContactId == contactId && gs.DatePlayed <= beforeDate
									orderby gs.DatePlayed descending
									select gs).Take(40);

				handIndex = CalculateHandicapIndex(last40Rounds);
			}

			return handIndex;
		}

		public static double? CalculateHandicapIndex(long contactId, bool for9Holes = false)
		{
			double? handIndex = null;

            DB db = DBConnection.GetContext();

			if (for9Holes)
			{
				// 9 hole handicaps will only include 9 hole scores.
				var last20Rounds = (from gs in db.GolfScores
									where gs.ContactId == contactId && gs.HolesPlayed == 9
									orderby gs.DatePlayed descending
									select gs).Take(20);

				handIndex = CalculateHandicapIndex9(last20Rounds);
			}
			else
			{
				var last40Rounds = (from gs in db.GolfScores
									where gs.ContactId == contactId
									orderby gs.DatePlayed descending
									select gs).Take(40);

				handIndex = CalculateHandicapIndex(last40Rounds);
			}

			return handIndex;
		}

		private static double? CalculateHandicapIndex9(IEnumerable<GolfScore> last20Rounds)
		{
			int count = last20Rounds.Count();
			if (count == 0)
				return null;

			int lowestScoresUsed = GetLowestScoresToUse(last20Rounds.Count());

			var diffs = (from gs in last20Rounds
						 select GolfScore.CalculateDifferential(gs.TotalESCScore(true), gs.GetRating() / 2.0, gs.GetSlope()));

			return CalculateHandicapIndex(diffs.OrderBy(o => o).Take(lowestScoresUsed).ToList());
		}

		private static double? CalculateHandicapIndex(IEnumerable<GolfScore> last40Rounds)
		{
			List<GolfScoreDouble> last20Rounds = new List<GolfScoreDouble>();

			GolfScore pending9HoleScore = null;

			foreach (GolfScore gs in last40Rounds)
			{
				if (gs.HolesPlayed != (int)GolfScore.eHolesPlayed.Eighteen)
				{
					if (pending9HoleScore == null)
					{
						pending9HoleScore = gs;
					}
					else
					{
						last20Rounds.Add(new GolfScoreDouble(pending9HoleScore, gs));
						pending9HoleScore = null;
					}
				}
				else
				{
					last20Rounds.Add(new GolfScoreDouble(gs));
				}

				if (last20Rounds.Count == 20)
					break;
			}

			if (last20Rounds.Count == 0)
				return null;

			int lowestScoresUsed = GetLowestScoresToUse(last20Rounds.Count);


			// sort scores and take lowest.
			var lowestScores = (from gd in last20Rounds
								orderby gd.Differential ascending
								select gd.Differential).Take(lowestScoresUsed).ToList();

			return CalculateHandicapIndex(lowestScores);
		}

		public static int GetLowestScoresToUse(int numScores)
		{
			int lowestScoresUsed = 0;

			if (numScores <= 6)
				lowestScoresUsed = 1;
			else if (numScores <= 8)
				lowestScoresUsed = 2;
			else if (numScores <= 10)
				lowestScoresUsed = 3;
			else if (numScores <= 12)
				lowestScoresUsed = 4;
			else if (numScores <= 14)
				lowestScoresUsed = 5;
			else if (numScores <= 16)
				lowestScoresUsed = 6;
			else if (numScores == 17)
				lowestScoresUsed = 7;
			else if (numScores == 18)
				lowestScoresUsed = 8;
			else if (numScores <= 19)
				lowestScoresUsed = 9;
			else if (numScores == 20)
				lowestScoresUsed = 10;

			return lowestScoresUsed;
		}

		private static double CalculateHandicapIndex(ICollection<double> diffScores)
		{
			double totalDiffs = 0.0;

			foreach (double diffScore in diffScores)
				totalDiffs += diffScore;

			return GolfScore.CalculateIndex(totalDiffs, diffScores.Count);
		}

	}
}

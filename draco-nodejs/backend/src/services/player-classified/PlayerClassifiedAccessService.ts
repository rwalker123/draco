...
export class PlayerClassifiedAccessService {
  ...
  isValidAccessCodeFormat(accessCode: string): boolean {
    ...
    // Basic format check for UUID v4 pattern
  ...
  /**
   * Get contact permissions for classified operations
   *
   * Retrieves comprehensive permission information for a contact within an account.
   * This includes role-based permissions and any special classified permissions.
   *
   * @param contactId - ID of the contact to get permissions for
   * @param accountId - Account ID for permission scope
   * @returns Object containing permission flags for different operations
   *
   * @security Centralizes permission logic to ensure consistency across operations
   *
   * @example
   * ```typescript
   * const permissions = await accessService.getContactPermissions(789n, 123n);
   * if (permissions.canManageClassifieds) {
   *   // Allow classified management operations
   * }
   * ```
   */
  async getContactPermissions(
    ...
    canViewPlayersWanted: boolean;
    canManageClassifieds: boolean;
    canEditAnyClassified: boolean;
    canDeleteAnyClassified: boolean;
    roles: string[];
  }> {
    try {
      // Get user roles
      ...
      const isAdmin = roleIds.includes('Administrator');
      const isAccountAdmin = roleIds.includes('AccountAdmin');
      const isContactAdmin = roleIds.includes('ContactAdmin');
      const hasManagementRole = isAdmin || isAccountAdmin || isContactAdmin;

      return {
        canCreatePlayersWanted: true, // All authenticated users can create
        canViewPlayersWanted: true, // All account members can view
        canManageClassifieds: hasManagementRole,
        canEditAnyClassified: isAdmin || isAccountAdmin,
        canDeleteAnyClassified: isAdmin || isAccountAdmin,
        roles: roleIds,
      };
    } catch (error) {
      logSecurely('error', 'Error getting contact permissions', {
        error: error instanceof Error ? error.message : String(error),
        contactId: String(contactId),
        accountId: String(accountId),
      });
      ...
      return {
        canCreatePlayersWanted: false,
        canViewPlayersWanted: false,
        canManageClassifieds: false,
        canEditAnyClassified: false,
        canDeleteAnyClassified: false,
        roles: [],
      };
    }
  }

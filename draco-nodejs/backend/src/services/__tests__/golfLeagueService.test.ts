...
function createMockLeagueSeason(overrides: Partial<leagueseason> = {}): leagueseason {
  return {
    id: 1n,
    leagueid: 1n,
    seasonid: 1n,
...
describe('GolfLeagueService', () => {
  ...
  beforeEach(() => {
    setups = [createMockLeagueSetup({ id: 1n, accountid: 100n, leagueseasonid: 1n })];
    ...
    repository = {
      ...
      ): Promise<GolfLeagueSetupWithOfficers | null> {
        return setups.find((s) => s.leagueseasonid === leagueSeasonId) ?? null;
      },
      async getLeagueSeasonId(accountId: bigint, seasonId: bigint): Promise<bigint | null> {
        if (accountId === 100n && seasonId === 1n) return 1n;
        if (accountId === 100n && seasonId === 2n) return 2n;
        return null;
      },
      ...
      async update(leagueSeasonId: bigint, data): Promise<golfleaguesetup> {
        const index = setups.findIndex((s) => s.leagueseasonid === leagueSeasonId);
        if (index === -1) {
  ...
  describe('getLeagueSetup', () => {
    it('returns formatted league setup when found', async () => {
      const result = await service.getLeagueSetup(100n, 1n);
    ...
    it('throws NotFoundError when setup not found for leagueSeasonId', async () => {
      await expect(service.getLeagueSetup(100n, 999n)).rejects.toBeInstanceOf(NotFoundError);
    });
  });

  describe('updateLeagueSetup', () => {
    it('updates league day', async () => {
      const result = await service.updateLeagueSetup(100n, 1n, { leagueDay: 4 });

      expect(result.leagueDay).toBe(4);
    });

    it('updates first tee time', async () => {
      const result = await service.updateLeagueSetup(100n, 1n, { firstTeeTime: '09:30' });

      expect(result.firstTeeTime).toBe('09:30');
    });

    it('updates time between tee times', async () => {
      const result = await service.updateLeagueSetup(100n, 1n, { timeBetweenTeeTimes: 15 });

      expect(result.timeBetweenTeeTimes).toBe(15);
    });

    it('updates holes per match', async () => {
      const result = await service.updateLeagueSetup(100n, 1n, { holesPerMatch: 18 });

      expect(result.holesPerMatch).toBe(18);
    });

    it('updates tee off format', async () => {
      const result = await service.updateLeagueSetup(100n, 1n, { teeOffFormat: 1 });
    ...
    it('updates officer IDs', async () => {
      await service.updateLeagueSetup(100n, 1n, {
        presidentId: '10',
        vicePresidentId: '20',
      });
    ...
    it('clears officer when set to empty string', async () => {
      ...
      await service.updateLeagueSetup(100n, 1n, { presidentId: '' });

      expect(setups[0].presidentid).toBeNull();
    });

    it('creates new setup when not found for leagueSeasonId', async () => {
      const result = await service.updateLeagueSetup(100n, 2n, { leagueDay: 1 });
    ...
    it('handles partial updates', async () => {
      const original = await service.getLeagueSetup(100n, 1n);
      const result = await service.updateLeagueSetup(100n, 1n, { leagueDay: 5 });

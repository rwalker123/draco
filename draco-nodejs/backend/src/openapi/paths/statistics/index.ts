...
export const registerStatisticsEndpoints = ({ registry, schemaRefs }: RegisterContext) => {
  ...
  registry.registerPath({
    method: 'get',
    path: '/api/accounts/{accountId}/seasons/{seasonId}/teams/{teamSeasonId}/schedule',
    operationId: 'listTeamSeasonSchedule',
    summary: 'List team season schedule',
    description:
      'Retrieve the full schedule for a team season with optional date range, recap, and pagination filters.',
    tags: ['Statistics'],
    parameters: [
      {
        name: 'accountId',
    ...
    responses: {
      200: {
        description: 'Team schedule retrieved',
        content: {
          'application/json': {

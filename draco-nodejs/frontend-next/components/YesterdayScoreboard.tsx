...
const YesterdayScoreboard: React.FC<YesterdayScoreboardProps> = ({ accountId, teamId }) => {
  ...
  const handleSaveGameResults = async (gameData: GameResultData) => {
    ...
    try {
      ...
      await loadYesterdayGames().then(setGames);
    } catch (err) {
  ...
  // Function to load yesterday's games
  ...
  }, [accountId, teamId, loadYesterdayGames]);
  ...
  const sections: GameListSection[] = [{ title: 'Yesterday', games: games }];

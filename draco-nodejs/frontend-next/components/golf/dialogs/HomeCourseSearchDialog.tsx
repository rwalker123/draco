...
interface HomeCourseSearchDialogProps {
  open: boolean;
  onClose: () => void;
...
export const HomeCourseSearchDialog: React.FC<HomeCourseSearchDialogProps> = ({
  open,
  onClose,
  onSelectCourse,
  accountId,
    ...
    try {
      ...
      if (selectedCourse.courseId && selectedCourse.externalId === '') {
        ...
        if (result.success) {
          ...
          setError(result.error ?? 'Failed to set home course');
        }
        return;
      }
  ...
  return (
    <Dialog
      ...
      maxWidth="md"
      fullWidth
      aria-labelledby="home-course-search-dialog-title"
    >
      <DialogTitle id="home-course-search-dialog-title" sx={{ pr: 6 }}>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          <Typography variant="body2" color="text.secondary">
            Search our database of 30,000+ golf courses worldwide to set your home course.
          </Typography>
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        ...
        >
          {saving ? 'Setting Home Course...' : 'Set as Home Course'}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default HomeCourseSearchDialog;

...
export function useBaseballLiveScoringOperations(): UseBaseballLiveScoringOperationsReturn {
  ...
  const checkSessionStatus = async (gameId: string): Promise<BaseballLiveSessionStatus | null> => {
  ...
  const getSessionState = async (gameId: string): Promise<BaseballLiveScoringState | null> => {
    if (!currentAccount?.id) {
      setError('No account selected');
      return null;
    }
  ...
  const startSession = async (gameId: string): Promise<BaseballLiveScoringState | null> => {
    if (!currentAccount?.id) {
      setError('No account selected');
      return null;
    }
  ...
  const advanceInningOp = async (gameId: string, inningNumber: number): Promise<boolean> => {
    if (!currentAccount?.id) {
      setError('No account selected');
      return false;
    }
  ...
  const finalizeSession = async (gameId: string): Promise<boolean> => {
    if (!currentAccount?.id) {
      setError('No account selected');
      return false;
    }
  ...
  const stopSession = async (gameId: string): Promise<boolean> => {
    if (!currentAccount?.id) {
      setError('No account selected');
      return false;
    }

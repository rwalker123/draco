...
export function useTeamManagement({ accountId, seasonId }: UseTeamManagementOptions) {
  ...
  const service = useMemo(() => new TeamManagementService(token, apiClient), [token, apiClient]);

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const updateTeamMetadata = useCallback(
    async (params: UpdateTeamMetadataParams): Promise<UpdateTeamMetadataResult> => {
      setLoading(true);
      setError(null);
    ...
    [accountId, seasonId, service, token],
  );

...
export const useUserDataManager = (dispatch: React.Dispatch<PaginationAction>): UserDataManager => {
  const setLoading = (isPaginating = false, page?: number) => {
  ...
  const setData = (users: ContactType[], hasNext: boolean, hasPrev: boolean, page?: number) => {
    dispatch({
      type: 'SET_DATA',
      users,
      hasNext,

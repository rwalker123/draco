...
export function useGameRecapFlow<GameType extends RecapGameBase>(
  ...
  const defaultDetermineEditableTeams = (game: GameType): string[] => {
    if (
  ...
  const determineEditableTeams = (game: GameType): string[] => {
  ...
  const resolveGameDate = (game: GameType): string | undefined => {
    if (game.gameDate) {
  ...
  const resolveHomeScore = (game: GameType): number | undefined =>
    typeof game.homeScore === 'number' ? game.homeScore : undefined;

  const resolveVisitorScore = (game: GameType): number | undefined =>
    typeof game.visitorScore === 'number' ? game.visitorScore : undefined;

  const resolveHomeTeamName = (game: GameType): string | undefined => {
    return game.homeTeamName ?? resolveTeamName(game, game.homeTeamId);
  };

  const resolveVisitorTeamName = (game: GameType): string | undefined => {
    return game.visitorTeamName ?? resolveTeamName(game, game.visitorTeamId);
  };

  const fetchRecapsForBothTeams = async (game: GameType) => {
    const teams = [
  ...
  const openEditRecap = async (game: GameType) => {
  ...
  const openViewRecap = async (game: GameType) => {
  ...
  const handleRecapSuccess = (result: UpsertGameRecapType) => {
    if (!dialogState) {
      return;
    }
  ...
  const canEditRecap = (game: GameType) => determineEditableTeams(game).length > 0;

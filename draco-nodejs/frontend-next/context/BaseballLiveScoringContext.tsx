...
export function BaseballLiveScoringProvider({ children }: BaseballLiveScoringProviderProps) {
  ...
  const connectWithTicket = async (gameId: string, ticket: string) => {
  ...
  const connect = (gameId: string, role: SseRole = 'watcher') => {
  ...
  const stableConnect = (gameId: string, role?: SseRole) => {
  ...
  const onInningAdvanced = (callback: (event: InningAdvancedEvent) => void) => {

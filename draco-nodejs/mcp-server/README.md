...
Draco MCP server — exposes Draco sports data to LLM clients (Claude, ChatGPT, Cursor, etc.) via the Model Context Protocol.
...
| `list_my_accounts` | All Draco accounts the user belongs to |
...
| `BACKEND_BASE_URL` | Yes | — | Draco backend URL (e.g. `http://localhost:3001`) |
...
| `OAUTH_RESOURCE_METADATA_URL` | No | `http://localhost:3001/.well-known/oauth-protected-resource` | Used in WWW-Authenticate headers |
...
3. Obtain a token via the OAuth flow:
   - Register a client via `POST http://localhost:3001/oauth/register`
   - Complete the authorization code + PKCE flow at `http://localhost:3001/oauth/authorize`
   - Exchange the code at `POST http://localhost:3001/oauth/token`

...
export class ApiClientError extends Error {
  ...
  constructor(message: string, options: { status?: number; response?: Response; details?: ApiError } = {}) {
    super(message);
...
const throwApiError = <T>(result: ApiResult<T>, fallbackMessage: string): never => {
...
export const assertNoApiError = (result: ApiResult<unknown>, fallbackMessage: string): void => {
  if (result.error) {
